...
//
//Created by 16307015 on 18-10-29.
...
int snakeAIX[SNAKE_MAX_LENGTH] = { 10,9,8,7,6 };
int snakeAIY[SNAKE_MAX_LENGTH] = { 10,10,10,10,10 };
...
// 返回 movable[p]
	...
	int dir=0;
	int Hx=snakeAIX[snakeAILength-1],Hy=snakeAIY[snakeAILength-1];
	for(int i=0;i<4;i++){
...
int main()
{
		...
		int end=0;
        while (!end) {
		printf("玩家在左上角，AI蛇在右下角，谁先撞到除食物外的物体就输了，\n如果对方先吃到10个食物你也输了按下S后开始游戏(q结束):");
		char st = getch();
		if(st=='q'||st=='Q'){
				end=1;
				break;
			...
			while (aialive&&alive&&!end) {
				...
				if (kbhit()) {
					...
					switch (dir) {
						...
						break;
						default:
						snakeMove(0,curv, curh);
					}
				}
				else snakeMove(0,curv, curh);
...
void init() {
	...
	for (int i = 1; i < 11; ++i)
		for (int j = 1; j < 11; ++j)
			map[i][j] = ' ';
	map[1][1] = map[1][2] = map[1][3] = map[1][4] = map[10][10] = map[10][9] = map[10][8] = map[10][7] = SNAKE_BODY;
	map[1][5] = map[10][6] = SNAKE_HEAD;
	for (int i = 0; i < 5; ++i) {
		snakeX[i] = i + 1;
		snakeY[i] = 1;
		snakeAIX[i]= 10-i;
...
void snakeMove(int ai,int v, int h) {
	int newrow,newcol;
	if(ai){
		newrow = snakeAIY[snakeAILength - 1] + v;
		newcol = snakeAIX[snakeAILength - 1] + h;
	}
	else{
		newrow = snakeY[snakeLength - 1] + v;
		newcol = snakeX[snakeLength - 1] + h;
	}
	//若下一步为墙则失败，游戏结束
	...
	//若下一步为蛇身且方向不是当前方向的反方向则失败，游戏结束
		...
		//长度未到最大长度时继续游戏
		if(ai){
			if (snakeAILength < SNAKE_MAX_LENGTH) {
				//修改当前移动方向
				curav = v;
				curah = h;
				//如果下一步为空格时正常移动
				if (map[newrow][newcol] == BLANK_CELL) {
					map[snakeAIY[snakeAILength - 1]][snakeAIX[snakeAILength - 1]] = SNAKE_BODY;
					map[newrow][newcol] = SNAKE_HEAD;
					map[snakeAIY[0]][snakeAIX[0]] = BLANK_CELL;
					for (int i = 0; i < snakeAILength - 1; ++i) {
						snakeAIX[i] = snakeAIX[i + 1];
						snakeAIY[i] = snakeAIY[i + 1];
					}
					snakeAIX[snakeAILength - 1] = newcol;
					snakeAIY[snakeAILength - 1] = newrow; 
				}
				//若下一步为食物，蛇长度加一
				else if (map[newrow][newcol] == SNAKE_FOOD) {
					map[snakeAIY[snakeAILength - 1]][snakeAIX[snakeAILength - 1]] = SNAKE_BODY;
					map[newrow][newcol] = SNAKE_HEAD;
					map[snakeAIY[0]][snakeAIX[0]] = SNAKE_BODY;
					snakeAIX[snakeAILength] = newcol;
					snakeAIY[snakeAILength] = newrow;
					snakeAILength += 1;
					score += 10;
					put_money();
				}
